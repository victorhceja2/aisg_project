...
const CatalogSelector: React.FC = () => {
  ...
  const handleCatalogClick = (path: string, disabled: boolean) => {
    if (!disabled) {
      navigate(path);
    }
  };

  const handleKeyDown = (e: React.KeyboardEvent, path: string, disabled: boolean) => {
    if (e.key === 'Enter' || e.key === ' ') {
      e.preventDefault();
      handleCatalogClick(path, disabled);
    }
  };
    ...
    <AISGBackground>
      <div className="max-w-7xl mx-auto p-6 font-['Montserrat']">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
          {catalogs.map(({ label, path, icon, disabled }) => (
            <div
              key={path}
              onClick={() => handleCatalogClick(path, disabled)}
              onKeyDown={(e) => handleKeyDown(e, path, disabled)}
              ...
              className={`
                ...
                hover:shadow-2xl transition-all duration-200 border border-[#0033A0]
                flex flex-col items-center focus:outline-none focus:ring-2 focus:ring-[#00B140] focus:ring-offset-2 focus:ring-offset-transparent
                ${disabled
                  ? "opacity-60 cursor-not-allowed hover:border-[#0033A0]"
                  : "hover:border-[#00B140] hover:scale-105"}
              `}
            ...
            >
              ...
              <div className="p-6 text-center flex-1 flex flex-col justify-center">
                ...
                <p className="mt-2 text-sm text-gray-400">
                  {disabled ? "Not available" : "Click to manage this catalog"}
                </p>
              </div>
            </div>
          ))}
