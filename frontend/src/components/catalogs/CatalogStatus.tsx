...
const CatalogStatus: React.FC = () => {
  ...
  const confirmDelete = async (id: number, name: string) => {
    ...
    const { inUse } = await checkStatusUsage(id);
    ...
    if (inUse) {
      ...
      setDeleteErrorMessage(
        `Cannot delete service status "${name}" because it is currently being used in the system.`
      );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const { inUse } = await checkStatusUsage(statusToDelete.id);
      
      if (inUse) {
        // Si ahora está en uso, mostrar error simplificado
        setDeleteErrorMessage(
          `Cannot delete service status "${statusToDelete.name}" because it is currently being used in the system.`
        );
