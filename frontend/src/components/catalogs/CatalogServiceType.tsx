...
/**
 * Componente para mostrar y administrar el catálogo de tipos de servicio.
 * Permite buscar, agregar, editar y eliminar tipos de servicio.
 */
const CatalogServiceType: React.FC = () => {
    ...
    // Obtiene la lista de tipos de servicio desde el backend
    const fetchServiceTypes = async () => {
        setLoading(true);
        try {
            const res = await axios.get(
                `${apiURL}/catalog/service-type/${search ? `?search=${encodeURIComponent(search)}` : ""}`
            );
            ...
            <div className="min-h-screen p-6 font-['Montserrat']">
                <div className="max-w-4xl mx-auto">
                    <div className="bg-gradient-to-r from-[#0033A0] to-[#00B140] p-6 rounded-t-lg shadow-lg">
                        <h1 className="text-3xl font-bold text-white text-center">Service Types Catalog</h1>
                        <p className="text-gray-200 mt-2 font-light text-center">Manage the different service types</p>
                    </div>
                                        ...
                                        {serviceTypes.length === 0 ? (
                                            ...
                                            serviceTypes.map((t) => (
                                                <tr key={t.id_service_type} className="hover:bg-[#1E2A45] transition-colors">
                                                    <td className="px-6 py-4 text-gray-300">{t.id_service_type}</td>
                                                    <td className="px-6 py-4 text-white font-medium">{t.service_type_name}</td>
