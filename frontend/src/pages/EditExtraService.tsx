...
const EditExtraService: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (err.response) {
                setError(`Error (${err.response.status}): ${err.response.data.detail || "Could not update the assignment."}`);
            } else {
                setError("Could not update the assignment. Please check the data and try again.");
            }
            setIsLoading(false);
        }
    };
                    ...
                    <div className="bg-[#1E2A45] rounded-b-lg shadow-lg px-8 py-8">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex space-x-4 pt-4 justify-end">
                                ...
                                <button
                                    type="submit"
                                    className={`w-1/2 bg-gradient-to-r from-[#0033A0] to-[#00B140] hover:from-[#002D8A] hover:to-[#009935] text-white font-medium py-3 px-4 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg flex items-center justify-center ${isLoading ? "bg-gray-500" : ""}`}
                                    disabled={isLoading}
                                >
                                    {isLoading ? (
                                        <>
