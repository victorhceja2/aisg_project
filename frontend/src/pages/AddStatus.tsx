...
/**
 * Componente para agregar un nuevo estatus de servicio.
 * Permite ingresar el nombre del estatus y lo guarda en el backend.
 * Incluye validación, manejo de errores y navegación tras guardar o cancelar.
 */
const AddStatus: React.FC = () => {
    // Hook para navegación programática
    const navigate = useNavigate();
    // URL base de la API (usa variable de entorno o IP por defecto)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error("Error al crear estatus:", err);
            if (err.response) {
                setError(`Error (${err.response.status}): ${err.response.data.detail || "No se pudo crear el estatus"}`);
            } else {
                setError("No se pudo crear el estatus. Verifique su conexión e intente nuevamente.");
            }
            setIsLoading(false);
        }
    };
            ...
            <div className="max-w-2xl mx-auto">
                ...
                <div className="bg-gradient-to-r from-[#0033A0] to-[#00B140] p-6 rounded-t-lg shadow-lg mb-0">
                    ...
                    <p className="text-gray-200 mt-2 font-light text-center">
                        Crea un nuevo estatus para los servicios
                    </p>
                </div>
